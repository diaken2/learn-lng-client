// AudioPrepositionModal.jsx
import React, { useState, useEffect, useRef } from 'react';

const API_BASE_URL = 'https://learn-lng-new-client-lrqy.onrender.com/api';

export default function AudioPrepositionModal({ isOpen, onClose, word, language, onAudioSaved }) {
  ...
  // Генерируем уникальный ID для предлога
  const getWordId = () => {
    if (word?.imageBase) return word.imageBase;
    if (word?.russianWord) return `preposition_${word.russianWord.toLowerCase().replace(/[^a-zа-яё]/g, '_')}`;
    if (word?.displayWord) return `preposition_${word.displayWord.toLowerCase().replace(/[^a-zа-яё]/g, '_')}`;
    return `preposition_${Date.now()}`;
  };
  ...
  const loadExistingAudio = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/preposition-audio/${wordId}/${language}`);
  ...
  const uploadAudio = async () => {
    ...
    try {
      ...
      const urlResponse = await fetch(`${API_BASE_URL}/preposition-audio/generate-upload-url`, {
        method: 'POST',
      ...
      const saveResponse = await fetch(`${API_BASE_URL}/preposition-audio/save`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          wordId: wordId,
