// AudioAdjectiveModal.jsx
import React, { useState, useEffect, useRef } from 'react';

const API_BASE_URL = 'https://learn-lng-new-client-lrqy.onrender.com/api';

export default function AudioAdjectiveModal({ isOpen, onClose, word, language, onAudioSaved }) {
  ...
  useEffect(() => {
    if (isOpen && word?.imageBase && language) {
      loadExistingAudio();
    }
  ...
  const loadExistingAudio = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/adjective-audio/${word.imageBase}/${language}`);
  ...
  const uploadAudio = async () => {
    ...
    try {
      const fileName = `${word.imageBase}_${language}_${Date.now()}.${audioBlob.type.split('/')[1] || 'webm'}`;
      
      const urlResponse = await fetch(`${API_BASE_URL}/adjective-audio/generate-upload-url`, {
        method: 'POST',
      ...
      const saveResponse = await fetch(`${API_BASE_URL}/adjective-audio/save`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          imageBase: word.imageBase,
