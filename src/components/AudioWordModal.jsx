...
export default function AudioWordModal({ isOpen, onClose, word, language, onAudioSaved }) {
  ...
  // Загружаем существующее аудио при открытии
  useEffect(() => {
  ...
  const loadExistingAudio = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/word-audio/${word.imageBase}/${language}`);
  ...
  const uploadAudio = async () => {
    ...
    try {
      ...
      const urlResponse = await fetch(`${API_BASE_URL}/word-audio/generate-upload-url`, {
        method: 'POST',
      ...
      const saveResponse = await fetch(`${API_BASE_URL}/word-audio/save`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          imageBase: word.imageBase,
